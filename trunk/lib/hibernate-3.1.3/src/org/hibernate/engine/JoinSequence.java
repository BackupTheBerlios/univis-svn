//$Id: JoinSequence.java 9528 2006-03-01 04:24:08Z steve.ebersole@jboss.com $
...
public class JoinSequence {
	...
	public JoinFragment toJoinFragment(Map enabledFilters, boolean includeExtraJoins, String extraOnClause) throws MappingException {
		...
		for ( int i = 0; i < joins.size(); i++ ) {
			...
			if ( extraOnClause != null && !isManyToManyRoot( join.getJoinable() ) ) {
				// the isManyToManyRoot() check ensures we apply the extra on clause only to
				// the target table in the case of a many-to-many join
