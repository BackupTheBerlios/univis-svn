...
public class QueryPlanCache implements Serializable {
	...
	private ParameterMetadata buildNativeSQLParameterMetadata(String sqlString) {
		...
		Iterator itr = recognizer.getNamedParameterLocationMap().entrySet().iterator();
		Map namedParamDescriptorMap = new HashMap();
		while( itr.hasNext() ) {
			final Map.Entry entry = ( Map.Entry ) itr.next();
			final String name = ( String ) entry.getKey();
			final List locationList = ( List ) entry.getValue();
			namedParamDescriptorMap.put(
					name ,
			        new NamedParameterDescriptor( name, null, ArrayHelper.toIntArray( locationList ) )
			);
		}
